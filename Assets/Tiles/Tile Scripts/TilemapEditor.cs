...
public class TilemapEditor : MonoBehaviour
{
    ...
    public Vector2 StartedClickingPosition;
    private void Update()
    {
        ...
        {
            StartedClickingPosition = new Vector2(MousePos.x, MousePos.y);
        }
        ...
        } else
        {
            ...
            {
                if (InventorySlots[SelectedSlot].CanBreak)
                {
                    if (InventorySlots[SelectedSlot].BreakType == "Pickaxe" || InventorySlots[SelectedSlot].BreakType == "Axe")
                    {
                        if (GameManager.Instance.tileEditManager.worldTiles.ContainsKey(new Vector2(MousePos.x, MousePos.y)))
                        {
                            if (InventorySlots[SelectedSlot].BreakType == GameManager.Instance.tileEditManager.worldTiles[new Vector2(MousePos.x, MousePos.y)].TypeToBreak)
                            {
                                if (InventorySlots[SelectedSlot].BreakLevel >= GameManager.Instance.tileEditManager.worldTiles[new Vector2(MousePos.x, MousePos.y)].LvlToBreak)
                                {
                                    if (GameManager.Instance.tileEditManager.BreakTime <= (((60 * GameManager.Instance.tileEditManager.worldTiles[new Vector2(MousePos.x, MousePos.y)].Toughness) / InventorySlots[SelectedSlot].BreakSpeed) * 60))
                                    {
                                        GameManager.Instance.tileEditManager.CanBreak = false;
                                        GameManager.Instance.tileEditManager.BreakTime++;
                                    }
                                    else //if (GameManager.Instance.tileEditManager.BreakTime == (((60 * GameManager.Instance.tileEditManager.worldTiles[new Vector2(MousePos.x, MousePos.y)].Toughness) / InventorySlots[SelectedSlot].BreakSpeed) * 60))
                                    {
                                        GameManager.Instance.tileEditManager.CanBreak = true;
                                        GameManager.Instance.tileEditManager.BreakTime = 0;
                                    }
                    ...
                    else if (InventorySlots[SelectedSlot].BreakType == "Hammer")
                    {
                        if (GameManager.Instance.tileEditManager.worldWalls.ContainsKey(new Vector2(MousePos.x, MousePos.y)))
                        {

                            if (InventorySlots[SelectedSlot].BreakType == GameManager.Instance.tileEditManager.worldWalls[new Vector2(MousePos.x, MousePos.y)].TypeToBreak)
                            {
                                if (InventorySlots[SelectedSlot].BreakLevel >= GameManager.Instance.tileEditManager.worldWalls[new Vector2(MousePos.x, MousePos.y)].LvlToBreak)
                                {
                                    if (GameManager.Instance.tileEditManager.BreakTime < (((60 * GameManager.Instance.tileEditManager.worldWalls[new Vector2(MousePos.x, MousePos.y)].Toughness) / InventorySlots[SelectedSlot].BreakSpeed) * 60))
                                    {
                                        GameManager.Instance.tileEditManager.BreakTime++;
                                    }
                                    else if (GameManager.Instance.tileEditManager.BreakTime == (((60 * GameManager.Instance.tileEditManager.worldWalls[new Vector2(MousePos.x, MousePos.y)].Toughness) / InventorySlots[SelectedSlot].BreakSpeed) * 60))
                                    {
                                        GameManager.Instance.tileEditManager.CanBreak = true;
                                        GameManager.Instance.tileEditManager.BreakTime++;
                                    }
            ...
            if (Input.GetMouseButton(0))
            {
                if (InventorySlots[SelectedSlot] != null)
                {
                    ...
                    {
                    if (GameManager.Instance.tileEditManager.CanBreak)
                    {
                        if (GameManager.Instance.tileEditManager.worldTiles.ContainsKey(new Vector2(MousePos.x, MousePos.y)))
                        {
                            if (InventorySlots[SelectedSlot].BreakLevel >= GameManager.Instance.tileEditManager.worldTiles[new Vector2(MousePos.x, MousePos.y)].LvlToBreak)
